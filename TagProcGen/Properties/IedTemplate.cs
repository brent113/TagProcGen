    ...
    public class IedTemplate
    {
        ...
        public void Validate(RtacTemplate rtacTemplate)
        {
            ...
            if (tagNameTypeEntriesThatMapToSamePrototypeEntry.Count > 0)
                throw new Exception(string.Format("Template {0} contains a multiple tags of the same type: " + Microsoft.VisualBasic.Constants.vbCrLf + "{1} of type {2} and " + Microsoft.VisualBasic.Constants.vbCrLf + "{3} of type {4}.", xlSheet.Name, tagNameTypeEntriesThatMapToSamePrototypeEntry.First().ElementAt(0).IedTagName, tagNameTypeEntriesThatMapToSamePrototypeEntry.First().ElementAt(0).IedTagTypeName, tagNameTypeEntriesThatMapToSamePrototypeEntry.First().ElementAt(1).IedTagName, tagNameTypeEntriesThatMapToSamePrototypeEntry.First().ElementAt(1).IedTagTypeName
)
);
    ...
    ).Where(x =>
    {
        ...
        return !(parseSuccess && parseNumber >= -1 & parseNumber <= 1);
            ...
            if (filtersWithDevicesNotInTemplate.Count > 0)
                throw new Exception(string.Format("Tag \"{0}\" has an invalid filter that references a device not in the template." + Microsoft.VisualBasic.Constants.vbCrLf + Microsoft.VisualBasic.Constants.vbCrLf + "Filter: {1}.", filtersWithDevicesNotInTemplate.First().TagName, filtersWithDevicesNotInTemplate.First().FilterString
)
);
