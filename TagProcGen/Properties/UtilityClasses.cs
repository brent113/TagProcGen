...
/// </summary>
    ...
    /// <param name="ExpectedParameters">List of parameters that must be in the dictionary or an error will be thrown.</param>
        ...
        {
            while (!string.IsNullOrEmpty(Conversions.ToString(start.Value)))
            {
                dict[Conversions.ToString(start.Value)] = Conversions.ToString(start.get_Offset(0, 1).Text);
            ...
            foreach (var p in ExpectedParameters)
            {
                if (!dict.ContainsKey(p))
                    throw new Exception(Conversions.ToString("Unable to locate pointer." + Microsoft.VisualBasic.Constants.vbCrLf + Microsoft.VisualBasic.Constants.vbCrLf + "Missing: \"" + p + "\" from " + start.Parent.Name));
            }
        }
