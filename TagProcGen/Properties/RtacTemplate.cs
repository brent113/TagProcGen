...
/// </summary>
    ...
    /// <returns>TagInfo structure of valid tag.</returns>
        public ServerTagInfo ValidateTag(string iedTagName)
        {
            ...
            if (tagMapInfo == null)
                throw new ArgumentException("Unable to locate tag mapping." + Microsoft.VisualBasic.Constants.vbCrLf + Microsoft.VisualBasic.Constants.vbCrLf + "Missing: \"" + iedTagName + "\" in tag map.");

            var Tag = new ServerTagInfo(tagMapInfo.ServerTagTypeName);
            if (!RtacTagPrototypes.ContainsKey(Tag.RootServerTagTypeName))
                throw new ArgumentException("Unable to locate tag prototype." + Microsoft.VisualBasic.Constants.vbCrLf + Microsoft.VisualBasic.Constants.vbCrLf + "Missing: \"" + Tag.RootServerTagTypeName + "\" in tag prototype.");
    ...
    /// <param name="path">Source filename to append output suffix on.</param>
        private void WriteServerTagCSV(KeyValuePair<string, OutputList> type, string path)
        {
            ...
            if (!RtacTagPrototypes.ContainsKey(typeName))
                throw new ArgumentException("Unable to locate tag prototype." + Microsoft.VisualBasic.Constants.vbCrLf + Microsoft.VisualBasic.Constants.vbCrLf + "Missing: \"" + typeName + "\" in tag prototype.");

            string csvPath = System.IO.Path.GetDirectoryName(path) + Conversions.ToString(System.IO.Path.DirectorySeparatorChar) + System.IO.Path.GetFileNameWithoutExtension(path) + "_RtacServerTags_" + typeName + ".csv";
            using (var csvStreamWriter = new System.IO.StreamWriter(csvPath, false))
            {
                ...
                tagGroup.ForEach(x => x[RtacTagPrototypes[typeName].SortingColumn] = Conversions.ToString(Conversion.Fix(x[RtacTagPrototypes[typeName].SortingColumn])));
