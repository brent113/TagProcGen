...
/// </summary>
    ...
    /// <param name="keyAddress">Optional parameter to use a different address when generating a key. Do not apply any offset.</param>
        ...
        {
            int adjustedAddress = address + Conversions.ToInteger(Pointers[Constants.TPL_SCADA_ADDRESS_OFFSET]);
            keyAddress = keyAddress > 0 ? keyAddress + Conversions.ToInteger(Pointers[Constants.TPL_SCADA_ADDRESS_OFFSET]) : adjustedAddress;
    ...
    /// <param name="path">Source filename to append output suffix on.</param>
        private void WriteScadaTagCSV(KeyValuePair<string, OutputList> type, string path)
        {
            ...
            string csvPath = System.IO.Path.GetDirectoryName(path) + Conversions.ToString(System.IO.Path.DirectorySeparatorChar) + System.IO.Path.GetFileNameWithoutExtension(path) + "_ScadaTags_" + typeName + ".csv";
            using (var csvStreamWriter = new System.IO.StreamWriter(csvPath, false))
            {
                ...
                {
                    int i;
                    var r = new { Value = s, isString = !int.TryParse(s, out i) };
                    if (r.isString)
